...
function AnswerFields({ questionIndex, control, register }) {
    const t = useTranslations('education.questions.create')
    const {
            ...
            {answersFields.map((answers, i) => (
                <div className="flex gap-4 mt-4">
                    ...
                    <input type="text"   {...register(
                        `data.sub_questions_list.${questionIndex}.answers.${i}.text.ar`)} className="p-4 border-gray-200 rounded-lg text-sm placeholder:text-transparent focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none 
                                                focus:pt-6
                                                focus:pb-2"
                    />
                    <label>{t('answerAr')}</label>
                    <input type="text"    {...register(
                        `data.sub_questions_list.${questionIndex}.answers.${i}.text.fr`)} className="p-4 block border-gray-200 rounded-lg text-sm placeholder:text-transparent focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none 
                                                focus:pt-6
                                                focus:pb-2"
                    ...
                    {canRemoveAnswer && (
                        <button
                            className="flex items-center justify-end py-3 px-4  gap-x-2 text-sm font-bold rounded-lg border border-transparent bg-primary-600 text-white hover:bg-primary-700"
