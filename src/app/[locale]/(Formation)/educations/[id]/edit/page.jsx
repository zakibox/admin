...
const EditEducation = ({ params: {locale, id} }) => {
  const t = useTranslations('education.edit')
  const schema = z.object({
    data: z.object({
      ...
      questions_num: z.string().min(1, "enter a valid number"),
      content: z.object({
        fr: z.any(),
        ar: z.any(),
      }),
  ...
  const { register, handleSubmit, setValue , getValues ,errors} = useForm(
    {
      ...
      defaultValues: async () => {
        try {
          const response = await axios.get(`http://127.0.0.1:8000/api/educationRoutieres/${id}`);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await axios.put(`http://127.0.0.1:8000/api/educationRoutieres/${id}`, data.data);
  ...
  return (
    ...
    <Link href={`/${locale}/educations`}>
     <button className="px-3 py-3 rounded-full bg-primary-600 text-primary-100 mb-3">               
                            <ChevronLeftIcon
                                className="w-6 h-6"
                                strokeWidth="2.5"
                            />
        ...
        <div className="p-4 sm:p-7 flex flex-col bg-gray-100 rounded-2xl shadow-lg ">
          ...
          <div className="mt-5">
            <div className="grid grid-cols-1 gap-4">
              ...
              { 
                <img  src={getValues("data.img")} alt="Uploaded" className="w-20 h-20" />}
              ...
              <div>
                ...
                <div className="relative">
                  <input type="text" {...register("data.title.ar")}   className="peer p-4 block w-full border-gray-200 rounded-lg text-sm placeholder:text-transparent focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none 
              focus:pt-6
              focus:pb-2
              [&:not(:placeholder-shown)]:pt-6
              [&:not(:placeholder-shown)]:pb-2
              ...
              autofill:pb-2" placeholder="Preline" />
                ...
                <div className="relative py-4">
                  ...
                  <FroalaEditor  
                  model= {getValues("data.content.fr")}
                 ...
                 {errors?.data?.content?.fr && (<p className="text-red-500">content is required</p>)}
                  ...
                  <FroalaEditor  model={getValues("data.content.ar")}
